...
def process_sheet(file_path, sheet_name):
    ...
    # Wide -> Long
    df_long = df.melt(
        id_vars=["Kode", "Deskripsi", "Satuan", "Jenis"],
        var_name="periode",
        value_name="nilai",
    )
...
# Main ETL
# =========================
file_path = "pdb_final_input.xlsx"
...
try:
    ...
    # =========================
    print("Truncate table pdb...")
    cur.execute("TRUNCATE TABLE pdb RESTART IDENTITY;")
    conn.commit()

    # =========================
    # 2. Proses semua sheet otomatis
    # =========================
    all_sheets = ["ADHB", "ADHK"]

    insert_query = """
    INSERT INTO pdb
    (kode, deskripsi, satuan, jenis, tahun, freq, period, nilai, created_at)
